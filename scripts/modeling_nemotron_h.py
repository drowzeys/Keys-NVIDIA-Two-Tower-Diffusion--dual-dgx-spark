...
class NemotronHMamba2Mixer(nn.Module):
    ...
    def torch_forward(self, input_states, cache_params: Optional[HybridMambaAttentionDynamicCache]=None, cache_position:Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None):
        ...
        if cache_params is not None and cache_position is not None and cache_position[0] > 0:
            # We need to guarantee that anything regarding the cache is on the same device
            cache_device = cache_params.ssm_states[self.layer_idx].device  # per-layer list fix
...
class NemotronHMOE(nn.Module):
    ...
    def moe(self, hidden_states: torch.Tensor, topk_indices: torch.Tensor, topk_weights: torch.Tensor):
        """MoE: one batched matmul per active expert. Empty experts are skipped
        at inference; in training only, a 0-token dummy forward runs so DDP/grad
        hooks mark every expert's params as used (numerically inert: no biases).
        """
